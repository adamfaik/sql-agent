...
class ValidationFormat(BaseModel):
    """Schema for the semantic validation output."""
    is_answerable: bool = Field(description="True if the question can be answered strictly using the provided schema, False otherwise.")
    reason: str = Field(description="If False, explain politely in French why the data is missing. If True, leave empty.")
...
# 2.7 Define Node: Semantic Validator (Guardrail)
def validate_query(state: AgentState) -> dict:
    """
    Node: Evaluates if the standalone query can be answered using the database schema.
    Prevents hallucinations and out-of-scope SQL generation.
    """
    print("--- NODE: SEMANTIC VALIDATION ---")
    ...
    llm = ChatOpenAI(model="gpt-4o-mini", temperature=0)
    # Force the LLM to output the exact Pydantic JSON structure
    structured_llm = llm.with_structured_output(ValidationFormat)
    
    system_prompt = f"""You are a strict Data Governance Expert.
    Evaluate if the user's question can be answered using ONLY the provided database schema.
    Do NOT assume the existence of columns, tables, or external knowledge (like weather, employee salaries, competitors) not explicitly defined in the schema.
    ...
    if not result.is_answerable:
        ...
        # If rejected, we pre-fill the summary to directly answer the user without hitting the DB
        return {
            "is_answerable": False,
    ...
    return {"is_answerable": True, "validation_reason": ""}
...
def create_graph():
    ...
    # 2. Reformulate -> Validate
    workflow.add_edge("reformulate_query", "validate_query")
    
    # 3. Validate -> SQL (if True) OR End (if False)
    ...
    # 5. Execute -> SQL (if error) OR Summarize (if success)
    workflow.add_conditional_edges(
        "execute_sql",
        check_for_errors,
        {
